...
const props = defineProps<{ demo: TokenDemo }>();
...
<template>
	...
	>
		...
		<div class="mt-6 grid gap-6 md:grid-cols-2">
			<!-- 左栏：示例组选择 -->
			<div class="flex flex-col gap-2" role="group" aria-label="选择示例文本">
				<button
			...
			<div
				...
				aria-live="polite"
				aria-label="token 解析面板"
			>
				<template v-if="activeGroup">
					<h3 class="text-xs font-semibold tracking-wide text-zinc-400 uppercase">原文</h3>
					<p
					...
					<div class="mt-2 flex flex-wrap gap-0.5" aria-label="token 切分条">
						<span
					...
					<p class="mt-3 font-mono text-xs text-zinc-400">
						字符数 {{ charCount }}（按 code point 计）· token 数 {{ activeGroup.tokenCount }} · 平均
						{{ avgTokensPerChar }} token/字符
					</p>
					...
					<div class="mt-4 rounded-lg border border-zinc-800 bg-zinc-900/60 p-3">
						...
						<p class="mt-2 font-mono text-sm text-sky-300" aria-label="估算输入成本">
							输入成本 ≈ ¥{{ estimatedCost }}
						</p>
						<p class="mt-1 text-xs text-zinc-500">
							费率 ¥{{ demo.costPerMillion }}/百万 token（示例费率，非报价）
						</p>
					</div>

					<p class="mt-3 text-xs text-zinc-500">
						token 数与切分边界均为教学示意，非真实 tokenizer 输出。
					</p>
				</template>
			</div>
		</div>
